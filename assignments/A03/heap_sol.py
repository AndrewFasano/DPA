...
def free_enter(cpu, tb, hook):
    ...
    arg = panda.arch.get_arg(cpu, 1, convention='syscall')
...
def malloc_enter(cpu, tb, hook):
    ...
    current_asid = panda.current_asid(cpu)
    proc_name = panda.get_process_name(cpu)
    allocation_size = panda.arch.get_arg(cpu, 1, convention='syscall')
    ...
    ret_addr = panda.virtual_memory_read(cpu, reg_rsp, 8, fmt='int')
    ...
    def on_malloc_return(cpu, tb, h):
        ...
        return_value = panda.arch.get_retval(cpu, convention='syscall')
