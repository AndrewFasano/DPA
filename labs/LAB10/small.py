...
panda = Panda(generic="arm", extra_args="-D log.txt")
enabled = False
last_pc = None
run_idx = 0
last_pc = None
...
proc_map_cache = {} # (start, end): in target memory or not (not: library, stack, etc)
...
def on_branch(cpu, tb, idx):
    ...
    global proc_map_cache
    for (start, end) in proc_map_cache:
        if block_addr >= start and block_addr < end:
            break
    else:
        # Don't have mapping for this, let's look it up
        maps = panda.get_mappings(cpu)
        for map_entry in maps:
            proc_map_cache[(map_entry.base, map_entry.base+map_entry.size)] = \
                panda.ffi.string(map_entry.name).decode() == TARGET
            print(f"Set {map_entry.base:x} and co to {panda.ffi.string(map_entry.name).decode()} == {TARGET}")
    ...
    if not proc_map_cache[(start, end)]:
        #print(f"In {procname} we have a branch at {tb.pc:x} instruction {idx} but it's out of our object")
        return False

    node_name = hex(tb.pc)
    print(f"In {procname} we have a branch at {node_name} instruction {idx}")

    #if tb.pc >= 0x103fc and tb.pc < 0x10444:
    #    print(f"FLIP: {tb.pc:x}")
    #    return True
    return False
    ...
    if graph.out_degree(node_name) == 0:
        ...
        print(f"Half covered node at {node_name}", end="")
